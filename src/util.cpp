...
#else
    const char* pszModule = "EnterTainmentCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EnterTainmentCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EnterTainmentCoin
    // Mac: ~/Library/Application Support/EnterTainmentCoin
    // Unix: ~/.EnterTainmentCoin
    
    
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EnterTainmentCoin";
#else
    fs::path pathRet;
    ...
    if (pszHome == NULL || strlen(pszHome) == 0)
        ...
        return pathRet / "EnterTainmentCoin";
    #else
        // Unix
        return pathRet / ".EnterTainmentCoin";
    #endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "EnterTainmentCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "EnterTainmentCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong EnterTainmentCoin will not work properly.");
                    strMiscWarning = strMessage;
                    LogPrintf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("EnterTainmentCoin"), CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING);
                }
            }
        }
        if (fDebug) {
