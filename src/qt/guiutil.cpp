...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("EnterTainmentCoin"))
        return false;
...
{
    // Convert EnterTainmentCoin:// to EnterTainmentCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("EnterTainmentCoin://"))
    {
        uri.replace(0, 12, "EnterTainmentCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "EnterTainmentCoin.lnk";
}
...
{
    return GetAutostartDir() / "EnterTainmentCoin.desktop";
}
